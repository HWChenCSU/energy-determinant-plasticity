...
def set_init_syn(syn,init_weight):
    ...
    syn.g = 0
    syn.g_nmda = 0
    syn.Erest[:] = zeros(nr_syn)
    syn.Efire[:] = zeros(nr_syn)
    syn.PE[:] = zeros(nr_syn)
    syn.MEmax[:] = zeros(nr_syn)
    syn.MEdamp[:] = ones(nr_syn)
	
    syn.Erest_Free[:] = zeros(nr_syn)
    syn.Efire_Free[:] = zeros(nr_syn)
    syn.PE_Free[:] = zeros(nr_syn)
    syn.wampa_Free[:] = init_weight*ones(nr_syn)	
