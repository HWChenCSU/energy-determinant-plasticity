...
def get_swc(filename):
    ...
    * 7 = custom
        ...
        if line[0] != '#':  
            ...
            if (x,y,z) in segment_swc:
                ...
                if n > 0:
                    ...
                    segment_swc[(x,y,z)] = n
                    compStart[n] = (2*xhalf-x,2*yhalf-y,2*zhalf-z)
                    compEnd[n] = (x,y,z)
                else:
                    segment[(x,y,z)] = n
                    segment_swc[(x,y,z)] = n
    ...
    return segment,segment_swc,compStart,compEnd

        
        

def add_line(ax, lines, sec,linewdbasic=1.5):
...
def shiftedColorMap(cmap, start=0, midpoint=0.5, stop=1.0, name='shiftedcmap',
    ...
    for ri, si in zip(reg_index, shift_index):
        ...
        if ri>=cvec[indv] and ri<cvec[indv+1]:
            ...
            r,g,b,a = cmap(0.0)   #r,g,b,a = cmap(0.001)
        elif ri>=cvec[indv+1]:
            r,g,b,a = cmap(1.0)   #r,g,b,a = cmap(.999)
#        if indv==somspike-1:
#            r=.5
#            g=.5
#            b=.5
...
def modifiedColorMap(cmap, start=0, midpoint=0.5, stop=1.0, name='modifiedcmap'):
    '''
    Function to offset the "center" of a colormap. 
	Improved on function shiftedColorMap by Huanwen Chen
    '''
    cdict = {
        'red': [],
        'green': [],
    ...
    for ri, si in zip(reg_index, shift_index):
        if ri>0.4 and ri<0.5*0.999:
            ri=0.4
        elif ri>0.5*1.001 and ri<0.6:
            ri=0.6				
        elif ri>=0.5*0.999 and ri<=0.5*1.001:
            ri=0.5
