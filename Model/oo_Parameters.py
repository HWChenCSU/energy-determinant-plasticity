...
wa_max =  1.0            # maximal AMPA weight  1.5
wn_max =  1.0           # maximal NMDA weight  1.5
w_min = 0.0001		         # minimal NMDA weight  .0001
...
Theta_high = -15.*mV    # threshold for potentiation events	######Theta_high = -15.*mV!!!!!!!!!!!!!
Theta_low = Theta_low_Acker
x_reset = 5.            # spike trace reset value
taux = 20.*ms           # spike trace time constant
A_LTD = 4.e-4           # depression amplitude 		# 4.e-4
A_LTP = 14.e-4          # potentiation amplitude 	# 14.e-4
tau_ca = 5.*ms          # filter1 timeconstant
tau_ltd = 15.*ms          #filter2 timeconstant
tau_ltp = 45.*ms          #filter2 timeconstant tau_ltp = 45.*ms 
...
#-------------------------------------
ME_Ascale = 1.0    # 对ME_A的放大，比如为了节省时间，用5次运行结果作为60次运行结果时，此系数就为60/5=12

ME_A = 0.02
ME_Vrhigh = EL
ME_Ar = 0.2
MEmaxRatio = 175.0		# 单位时间的最大能量供给，实际单位为fJ/(um^2*second)
MEtau = 2.0*second    # 最大能量供给随时间衰减的时间常量。 MEtau = 1.5*second 
